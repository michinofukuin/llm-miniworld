...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_dir', type=str, default="./mujuco_log/Hopper-RM_PPO/model", help='model_dir')
    parser.add_argument('--log_dir', type=str, default="./mujuco_log/Hopper_PPO/", help='log_dir')
    ...
    if is_use_reward_machine:
        if is_use_transfered_reward:
            ...
            train_env = make_vec_env("Half-Cheetah-RM-v1", n_envs=4, seed=seed)
        ...
        if is_use_transfered_reward:
            ...
            train_env = make_vec_env("Half-Cheetah-v0", n_envs=4, seed=seed)
            # train_env = gym.make("Half-Cheetah-v0")
        else:
            method = "Original"
            train_env = make_vec_env("Half-Cheetah-v1", n_envs=4, seed=seed)
    ...
    eval_callback = EvalCallback(eval_env=test_env, best_model_save_path=model_dir, log_path=log_dir+"evaluate/",
                                 eval_freq=2000, n_eval_episodes=5, deterministic=True, render=False)
    model = PPO("MlpPolicy",
                train_env,
